...
class JSONeTest < Minitest::Test
  def a_hash
    { a: 1, b: 'foo', d: 'foo', c: { d: 'foo', e: [ 'foo', 'bar', 'baz' ] }, e: [ 'foo', 'bar' ] }.clone
  end
  ...
  def test_path
    assert_equal JSONe.raw_path('/tmp/foo.jsone'), '/tmp/foo.json'
    assert_equal JSONe.encrypted_path('/tmp/foo.json'), '/tmp/foo.jsone'
  end
  ...
  def test_encryption
    ...
    assert_equal enc_hash[:a], 1
    assert_equal enc_hash[:b].start_with?(JSONe::CIPHER_PREFIX), true
    assert_equal enc_hash[:c][:e].first.start_with?(JSONe::CIPHER_PREFIX), true
